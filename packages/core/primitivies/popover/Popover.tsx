...
export interface PublicPopoverProps
  extends PropsWithChildren<Omit<PopoverProps, 'displayArea' | 'arrowSize'>> {
...
export default class Popover extends Component<PublicPopoverProps, PublicPopoverState> {
  ...
  render(): ReactNode {
    ...
    if (from) {
      if (
        typeof from === 'object' &&
        ((from as Point).x || (from as Point).x === 0) &&
        ((from as Point).y || (from as Point).y === 0)
      ) {
        if (
          ((from as Rect).width || (from as Rect).width === 0) &&
          ((from as Rect).height || (from as Rect).height === 0)
        ) {
        ...
        if (isVisible === undefined) {
          sourceElement = React.cloneElement(from, {
        ...
        console.warn(
          'Popover: `from` prop is an invalid value. Pass a React element, Rect, RefObject, or function that returns a React element.'
        );
      }
    }
