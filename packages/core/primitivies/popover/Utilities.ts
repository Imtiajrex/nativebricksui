...
export function getRectForRef(ref: RefType): Promise<Rect> {
  return new Promise((resolve, reject) => {
    if (ref && ref?.current) {
      ...
      reject('getRectForRef - current is not set');
    }
  });
}
...
export function getArrowSize(placement: Placement, arrowStyle: StyleProp<ViewStyle>): Size {
