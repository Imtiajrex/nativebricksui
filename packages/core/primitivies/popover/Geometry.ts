...
import { getBorderRadius } from './Utilities';
...
export class Geometry {
  ...
  static equals(a: Geometry, b: Geometry): boolean {
    return (
      a.popoverOrigin.equals(b.popoverOrigin) &&
...
export function computeGeometry(options: ComputeGeometryProps): Geometry {
  ...
  if (fromRect && options.fromRect instanceof Rect) {
    ...
    if (Array.isArray(placement)) {
      ...
      const [bestProvidedPlacement] = placement.filter(
        (p) => p === Placement.AUTO || p === Placement.FLOATING || bestPlacements.includes(p)
      );
    ...
    if (
      ...
      const fromRectHeightVisible =
        fromRect.y < displayArea.y
      ...
      if (
        ...
        const preferredX = Math.max(
          fromRect.x + 10,
          fromRect.x + (fromRect.width - requestedContentSize.width) / 2
        );
        const preferredY = Math.max(
          fromRect.y + 10,
          fromRect.y + (fromRect.height - requestedContentSize.height) / 2
        );
  ...
  if (arrowShift && fromRect) {
    ...
    else newGeom.anchorPoint.y += arrowShift * 0.5 * fromRect.height;
  }
...
type SpaceListProps = Pick<
  ComputeGeometryDirectionProps,
  'fromRect' | 'displayArea' | 'arrowSize' | 'requestedContentSize'
>;
function generateSpaceList({
  ...
  function generateOption(
    props: Pick<PlacementOption, 'sizeRequested' | 'sizeAvailable'>
  ): PlacementOption {
    return {
      ...props,
...
function computeAutoGeometry(options: ComputeGeometryAutoProps): Geometry | null {
  const { displayArea, requestedContentSize, fromRect, previousPlacement, debug, arrowSize } =
    options;

  // Keep same placement if possible (left/right)
  if (previousPlacement === Placement.LEFT || previousPlacement === Placement.RIGHT) {
    const geom =
      previousPlacement === Placement.LEFT
  ...
  if (previousPlacement === Placement.TOP || previousPlacement === Placement.BOTTOM) {
    const geom =
      previousPlacement === Placement.TOP
...
function calculateBestPlacements(spaceList: SpaceList): Placement[] {
  return (Object.keys(spaceList) as Placement[])
    .filter((o) => spaceList[o]?.fits)
